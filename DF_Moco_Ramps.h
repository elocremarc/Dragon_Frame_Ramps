...
//RAMPS 1.4 
  const uint8_t ramps_step_pins[] = { A0, A6, 46, 26, 36, 49 };
  const uint8_t ramps_dir_pins[]  = { A1, A7, 48, 28, 34, 51 };
  const uint8_t ramps_en_pins[]   = { 38, A2, A8, 24, 30, 53 };
  
  
  #define MOTOR0_STEP_PORT PORTF 
  #define MOTOR0_STEP_PIN  B00000001  // PF0 Pin A0
  
  
  #define MOTOR1_STEP_PORT PORTF
  #define MOTOR1_STEP_PIN  B01000000 // PF6 Pin A6

  #define MOTOR2_STEP_PORT PORTL
  #define MOTOR2_STEP_PIN  B00001000 // PL3 D46

  #define MOTOR3_STEP_PORT PORTA
  #define MOTOR3_STEP_PIN  B00010000 // PA4 D26


  #define MOTOR4_STEP_PORT PORTC
  #define MOTOR4_STEP_PIN  B00000010 // PC1 D36
   
  #define MOTOR5_STEP_PORT PORTL
  #define MOTOR5_STEP_PIN  B00000001 // PL0 D49
...
{
  digitalWrite(ramps_en_pins[m], enable ? LOW : HIGH);
...
void setup()
{
  ...
  {
    // setup motor pins
	motors[i].stepPin = ramps_step_pins[i];
	motors[i].dirPin = ramps_dir_pins[i];
	pinMode(ramps_en_pins[i], OUTPUT);
	enableMotor(i, false);
